...
export function extractRecoilCallbackEvents(
  sourceFile: SourceFile,
  importMap: ImportMap,
  index: SymbolIndex,
): UsageEvent[] {
  return extractCallbackEvents(
    sourceFile,
    importMap,
    "recoil",
    "useRecoilCallback",
    parseRecoilCallbackBindings,
    classifyRecoilCallbackRead,
    classifyRecoilCallbackWrite,
    index,
  );
}

export function extractJotaiAtomCallbackEvents(
  sourceFile: SourceFile,
  importMap: ImportMap,
  index: SymbolIndex,
): UsageEvent[] {
  return extractCallbackEvents(
    ...
    "jotai/utils",
    "useAtomCallback",
    parseJotaiAtomCallbackBindings,
    classifyJotaiAtomCallbackRead,
    classifyJotaiAtomCallbackWrite,
    index,
  );
}
...
function extractCallbackEvents<TBindings>(
  ...
  hookModule: string,
  hookImported: string,
  parseBindings: (callbackFactory: CallbackFactoryFunction) => TBindings,
  classifyRead: (callExpression: CallExpression, bindings: TBindings, index: SymbolIndex) => UsageEvent | undefined,
  classifyWrite: (callExpression: CallExpression, bindings: TBindings, index: SymbolIndex) => UsageEvent | undefined,
  index: SymbolIndex,
): UsageEvent[] {
  ...
  for (const callExpression of sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    if (!factory || factory.module !== hookModule || factory.imported !== hookImported) {
      continue;
    }
